...
`pallet-content` does not persist parent/links/mentions/`ipfs_hash` in storage. These are
...
sequenceDiagram
  ...
  Chain->>Index: PublishRevision(item_id, owner, revision_id, links, mentions, ipfs_hash)
  Note right of Index: set revision=0 on create or +n on updates

  Chain->>Index: PublishRevision(item_id, owner, revision_id=1..n, links, mentions, ipfs_hash)
...
- Store `parents`, `links`, and `mentions` as arrays/lists per revision.
